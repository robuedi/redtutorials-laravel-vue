    ...
    <div class="login-info">
		<span> <!-- User image size is adjusted inside CSS, it should stay as is -->
			...
			<a href="javascript:void(0);" id="show-shortcut" data-action="toggleShortcut">
				<img src="{{url('/assets/_admin/')}}/img/avatars/male.png" alt="me" class="online" />
				<span>
					{{$user_name}}
				</span>
                ...
                <li class="@if (isset($menu['active']) && $menu['active']) active @endif">
                    <a href="@if (isset($menu['submenus']) && count($menu['submenus']) > 0)# @else /{{$menu['url']}}@endif">
                        <i class="fa fa-lg fa-fw {{ $menu['class'] }}">
                            @if($menu['name'] == 'Contact Messages' && $current_user_unread_msg_nr > 0 )
                                <em>{{$current_user_unread_msg_nr}}</em>
                            @endif
                        </i>
